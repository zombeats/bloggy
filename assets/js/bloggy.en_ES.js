...
(function() {
  ...
  if (window.newsletter_form) {
    $('#newsletter_form').attr('action', window.newsletter_form);
  }
...
(function() {
  $(function() {
    var i, j, len, len1, n, pageNumber, pageNumberi18n, property, ref, selector, selectors;
    selectors = ['.subscribe-banner', '.modal-title', '.modal-body p', '.modal-body .button-add.wide', '.read-more', '.older-posts', '.newer-posts', '.poweredby', 'footer .button-add.large.wide', 'a.post-meta', '.prefix-date'];
    ...
    if (Bloggy.is('page', 'post')) {
      ...
      for (j = 0, len1 = ref.length; j < len1; j++) {
        n = ref[j];
        pageNumber[n] = pageNumberi18n[n];
      }
      return $(selector).html(pageNumber.join(' '));
    }
  });
...
(function() {
  window.i18n = {
    ...
    '.subscribe-banner': '¿Te gustaría recibir el contenido en tu correo mensualmente? &nbsp;<a class="button" data-toggle="modal" data-target="#blogsubscribe">Suscríbete</a>',
    '.read-more': 'Leer Más',
    ...
    '.prefix-date': 'el',
    'footer .button-add.large.wide': 'Suscríbete y recíbelo semalmente en tu correo',
...
(function() {
  ...
  $(window).load(function() {
    var casperFullImg;
    return casperFullImg = (function() {
      $('img').each(function() {
        var contentWidth, imageWidth, method;
        contentWidth = $('.post-content').outerWidth();
        imageWidth = $(this)[0].naturalWidth;
        method = imageWidth >= contentWidth ? 'addClass' : 'removeClass';
        return $(this)[method]('full-img');
      });
      return $(this).smartresize(casperFullImg);
    })();
  });

  $(function() {
    $('.post-content').fitVids();
    if (!Bloggy.is('device', 'desktop')) {
      return FastClick.attach(Bloggy.app);
    }
  });
